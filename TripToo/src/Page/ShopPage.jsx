...
import allProducts from '../data/products'; // Default import
...
const ShopPage = () => {
  ...
  return (
    <div className="bg-[#f0f2f5] text-[#333] min-h-screen">
      <div className="bg-[#1a5b4f] text-white py-12 px-8 flex flex-col md:flex-row items-center justify-between">
        <div className="text-left mb-4 md:mb-0">
          <h1 className="text-4xl font-bold mb-2">Shop</h1>
          <p className="text-2xl font-semibold">All Kits</p>
        </div>
        <div className="relative w-full md:w-auto">
          <img src="/assets/shop-hero-image.png" alt="All Kits" className="w-full max-w-sm md:max-w-md mx-auto rounded-lg shadow-lg" />
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h2 className="text-3xl font-bold mb-8 text-[#0f1c2e]">All Kits</h2>
        <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-lg shadow-md mb-8">
          <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6 w-full md:w-auto">
            <div className="flex items-center space-x-2 w-full sm:w-auto">
              ...
              <select id="filter" value={filter} onChange={(e) => setFilter(e.target.value)} className="p-2 border border-gray-300 rounded-md bg-white text-gray-800 w-full">
                <option value="All Kits">All Kits</option>
              </select>
            </div>
            <div className="flex items-center space-x-2 w-full sm:w-auto">
              <label htmlFor="sort" className="font-medium text-gray-700">Sorty Sontis:</label>
              <select id="sort" value={sort} onChange={(e) => setSort(e.target.value)} className="p-2 border border-gray-300 rounded-md bg-white text-gray-800 w-full">
          ...
          <div className="flex items-center space-x-2 mt-4 md:mt-0 w-full md:w-auto">
            <input type="text" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="p-2 border border-gray-300 rounded-md bg-white text-gray-800 flex-grow" />
            <button className="bg-[#1a5b4f] hover:bg-[#206f61] text-white px-4 py-2 rounded-md font-semibold">View</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {displayedProducts.map((product) => (
            <div key={product.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-md flex flex-col transition-transform duration-200 hover:scale-[1.02]">
              ...
              <h3 className="font-bold text-lg mb-1 text-[#0f1c2e]">{product.title}</h3>
              ...
              <div className="flex justify-between items-center mt-auto pt-2 border-t border-gray-200">
                <span className="font-bold text-xl text-[#0f1c2e]">${product.price.toFixed(2)}</span>
          ...
          {displayedProducts.length === 0 && (
            <p className="col-span-full text-center text-gray-600 text-lg py-10">No products found matching your criteria.</p>
          )}
        </div>
      </div>
    </div>
